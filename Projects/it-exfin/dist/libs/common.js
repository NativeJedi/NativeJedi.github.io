window.onload = function() {
	...
	$('.btn-show').on('click', function() {
		$('.popup').animate({
			...
			$('.b-region-list').css('display', 'flex').animate({
				opacity: 1
			}, 300);
		});
	});
	...
	(function($) {
		...
		setTimeout(newFadeIn, 4000);

		$('.new__close').on('click', function(e) {
			$(this).parent().fadeOut();
		});
